...
COPY anland-build/Neon/*.deb /tmp/anland-build/Neon/
...
RUN sed -i 's/Components: main/Components: main restricted universe multiverse/g' /etc/apt/sources.list.d/ubuntu.sources 2>/dev/null || \
    ...
    # 配置 KDE Neon APT 密钥与源
    wget -qO- 'https://archive.neon.kde.org/public.key' | gpg --dearmor -o /usr/share/keyrings/kde-neon-archive-keyring.gpg && \
    echo "deb [signed-by=/usr/share/keyrings/kde-neon-archive-keyring.gpg] http://archive.neon.kde.org/user noble main" > /etc/apt/sources.list.d/neon.list && \
    echo "deb-src [signed-by=/usr/share/keyrings/kde-neon-archive-keyring.gpg] http://archive.neon.kde.org/user noble main" >> /etc/apt/sources.list.d/neon.list && \
    # 设置 KDE Neon 仓库优先级为最高，使其包覆盖 Ubuntu 原生包
    printf "Package: *\nPin: origin archive.neon.kde.org\nPin-Priority: 1100\n" > /etc/apt/preferences.d/99-neon && \
...
RUN apt-get update && \
    ...
    if [ "$BUILD_KDE" = "conc" ]; then \
        ...
        polkit-kde-agent-1 libpam-systemd libpam-modules libpam-kwallet5 plasma-workspace-x11 language-pack-kde-zh-hans language-pack-zh-hans qt6-translations-l10n; \
    fi && \
    # mobile版KDE
...
RUN if [ "$ENABLE_binfmt_ARG" = "true" ]; then \
        ...
        sed -i '/^Types: deb/a Architectures: arm64' /etc/apt/sources.list.d/ubuntu.sources 2>/dev/null || true && \
